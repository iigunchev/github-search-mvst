import * as data from '../services/searchUser';
const apiUser = {
  login: 'iigunchev',
  id: 86871642,
  ...
  following_url:
    'https://api.github.com/users/iigunchev/following{/other_user}',
...
const fetchMock = jest
  .spyOn(data, 'searchUser')
  .mockImplementation(() => Promise.resolve(apiUser));


test('returns a user from the GitHub API', async () => {
  const user = await fetchMock('username');
